...
hardturnleft = 44
hardturnright = 6
rateofchange = 1 # 1 cm per interval
...
#    if (distance < variance * (1 + averagedistance)) and (distance > variance * (1 - averagedistance)):
            ...
            averagedistance = sum(distancesamples) / samples
            print("Sonar 1", (previousdistance - distance), previousdistance, distance, distancesamples, int(averagedistance))
        averagedistance = distance
        
        pz.forward(speed)
        if (previousdistance - averagedistance) >= rateofchange:
            pz.setMotor(0, speed - 15)
            print("Right")

        if (averagedistance - previousdistance) >= rateofchange:
